...
def natural_cubic_spline_interpolation_method(points_list, x_to_find):
    """
    Finding point based on the sent x value using Natural Cubic Spline Interpolation Method
    ...
    for i in range(len(points_list) - 1):
        ...
        if points_list[i][0] <= x_to_find <= points_list[i + 1][0]:
            ...
            # Setting the solution to start from index 1 instead of 0, for easier formula calculation
            ...
            y_value = ((points_list[i + 1][0] - x_to_find) ** 3 * vector_solution[i][0] + (
                        x_to_find - points_list[i][0]) ** 3 * vector_solution[i + 1][0]) / (6 * h[i]) + (
                                  (points_list[i + 1][0] - x_to_find) * points_list[i][1] + (
                                      x_to_find - points_list[i][0]) * points_list[i + 1][1]) / h[i] - (((points_list[
                                                                                                              i + 1][
                                                                                                              0] - x_to_find) *
                                                                                                         vector_solution[
                                                                                                             i][0] + (
...
def build_data_table(points_list):
    ...
    # Initialize the last index of the array to zero (for easier adjustment)
    h.append(0)
    lam.append(0)
    u.append(0)
    d.append(0)
...
if __name__ == "__main__":
    ...
    # Running the program
    print('---------- Natural Cubic Spline Interpolation Method ----------')
    natural_cubic_spline_interpolation_method(graph_points, x_value)
